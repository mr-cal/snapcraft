...
def _run(cmd: List[str]) -> None:
    ...
    try:
        ...
        raise errors.SnapcraftError(message=command_string) from call_error


class DebugSplitter:
    def __init__(self, *, debug_dir: Path) -> None:
        self.objcopy_cmd = "objcopy"
        self.strip_cmd = "strip"
    ...
    def split(self, elf_file: ElfFile) -> Optional[Path]:
        ...
        if not elf_file.has_debug_info:
            emit.debug(f"No debug info found for {elf_file.path!r}.")
            return None
...
def split_debug_info(
    *, debug_dir: str, prime_dir: str, file_paths: Set[str]
    ...
    emit.debug(f"Collected debug information may be found in: {debug_dir}")

    collector = DebugSplitter(debug_dir=Path(debug_dir))
    for file_path in file_paths:
        file_path = os.path.join(prime_dir, file_path)
        if not ElfFile.is_elf(Path(file_path)):
            continue

        emit.debug(f"Checking debug info for: {file_path!r}")

        debug_file = collector.split(ElfFile(path=Path(file_path)))
